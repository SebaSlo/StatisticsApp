...
namespace StatisticsApp.Model
{
	...
	public class FrequencyTable<T> : IEnumerable<FrequencyTableEntry<T>> where T : IComparable<T>
	{
		...
		private Dictionary<T, double> _relFrequencies;
		...
		public FrequencyTable()
		{
            ...
            _relFrequencies = new Dictionary<T, double>();
              _length = 0;
			_count = 0;
			_description = "";
			_tag = null;
		...
		private void UpdateSumAndMean(T value)
		{
			...
			else
			{
				...
				try
				{
					_dblSum += (double)Convert.ChangeType(value, typeof(Double));
		...
		private double ComputeMoment(int r)
		{
			...
			try
			{
				...
				{
					_result += Math.Pow((double)Convert.ChangeType(entry.Value, typeof(Double)) - _mean, r) * (double)entry.AbsoluteFreq;
				}
			}
			catch
			{
		...
		private bool KS_Test(out double p)
		{
			...
			try
			{
				...
				{
					data[i] = (double)Convert.ChangeType(entry.Value, typeof(Double));
					i++;
				}
			}
			catch
		...
		public double Median
		{
			get
			{
				if (!Double.IsNaN(_mean))
				{
					...
					if ((SampleSize % 2) == 0)
						return ((double)Convert.ChangeType(_data[(SampleSize - 1) / 2 + 1], typeof(Double)) + (double)Convert.ChangeType(_data[(SampleSize - 1) / 2], typeof(Double))) / 2.0;
					else
						return (double)Convert.ChangeType(_data[(SampleSize - 1) / 2], typeof(Double));
				}
				else
					return double.NaN;
			}
		...
		public double VariancePop
		{
			get
			{
				try
				{
					...
					foreach (FrequencyTableEntry<T> entry in table)
					{
						temp = (double)Convert.ChangeType(entry.Value, typeof(Double));
		...
		public double Range
		{
			get
			{
				...
				{
					return ((double)Convert.ChangeType(Maximum, typeof(Double)) - (double)Convert.ChangeType(Minimum, typeof(Double)));
				}
				else
					return double.NaN;
			}
